...
<?php
    ...
    if(isset($_GET['action']) == 'delete'){
        ...
        $query = mysqli_query($conn, "SELECT * FROM usuarios WHERE id_usuario='$id_delete'");
        if(mysqli_num_rows($query) == 0){
            ...
            $delete = mysqli_query($conn, "DELETE FROM usuarios WHERE id_usuario='$id_delete'");
            if($delete){
    ...
    if (isset($_GET["e"])){
        ...
        if($error==1){
            ...
            <script type="text/javascript">
                history.pushState(null, "", "showUser.php");
            </script>
        <?php
        }
        elseif($error==2){
